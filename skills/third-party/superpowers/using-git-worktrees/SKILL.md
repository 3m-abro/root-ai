...
**If NOT ignored:** Use a safe external location or propose a task-scoped .gitignore change. Do not commit unrelated changes.
...
Inspect repository dependency manifests and lockfiles first. Select only the project-documented, task-authorized setup and test commands. Package installation and build hooks execute code; do not run them automatically because a manifest exists.
...
| Directory not ignored | Use external location or scoped ignore change |
